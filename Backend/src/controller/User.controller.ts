...
export default class UserController {
    public static async signup (req: Request, res: Response) {
        ...
        try {
            const user = await User.create({ name, email, password, confirm, regno, graduatingYear, major, bio })
    ...
    public static async forgotPassword (req: Request, res: Response) {
        ...
        try {
            ...
            await user.save();
            const link = 'https://' + process.env.frontendURL + '/user/reset/' + user.id + '/' + verifyHash
            await SendEmail.verify(email, 'Reset Password', `Reset your password at ${link}`)
            return res.status(200).json({
                success: true,
    ...
    public static async resendVerification (req: Request, res: Response) {
        ...
        try {
            ...
            user.verifyHash = verifyHash;
            await user.save();
            const link = 'https://' + process.env.frontendURL + '/user/verify/' + user.id + '/' + verifyHash
            await SendEmail.verify(email, 'Verify Email', `Verify your email at ${link}`)
            return res.status(200).json({
                success: true,
