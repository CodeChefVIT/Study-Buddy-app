...
exports.getQuiz = async (req, res) => {
    ...
    try {
        ...
        let Isgroup = await Groups.findById(quiz.group)
        if (Isgroup.members.indexOf(req.user.id) === -1)
            return res.status(400).json({ message: 'You are not a member of this group' })
        const { group, time, creator, questions, date, ...data } = quiz
        // filter questions to not send the answer
        let filteredQuestions = questions.map(question => { 
            return {
...
exports.createQuiz = async (req, res) => {
    ...
    try {
        // todo: Parse Time
        if (!(group && time && creator && questions)) 
            return res.status(400).json({ message: "Missing required fields" })
        let foundGroup = await Groups.findById(group)
        if (!foundGroup) 
            return res.status(400).json({ message: 'Group does not exist' })
        let foundUser = await User.findById(creator)
        if (!foundUser)
            return res.status(400).json({ message: 'User does not exist' })
        // check if the questions schema is valid
        for (let i = 0; i < questions.length; i++) {
            if (!questions[i].question || !questions[i].options || !questions[i].answer)
                return res.status(400).json({ message: 'Invalid questions schema' })
            // check if options include answer 
            if (questions[i].options.indexOf(questions[i].answer) === -1) 
                return res.status(400).json({ message: 'Answer is not included in options' })
        }
        let quiz = new Quiz({
            group,
            time,
...
exports.attemptQuiz = async (req, res) => {
    ...
    try {
        ...
        if (!quiz) 
            return res.status(400).json({ message: 'Quiz does not exist' })
        let UserInGroup = await Groups.findById(quiz.group)
        if (UserInGroup.members.indexOf(req.user.id) === -1)
            return res.status(400).json({ message: 'You are not a member of this group' })
        ...
        for (let i = 0; i < Questiondata.length; i++) {
            let question = quiz.questions.find(question => question.question === Questiondata[i].question)
            correct = false
            if (!question)
        ...
        let attempted = quiz.attempted.find(attempt => attempt.user === req.user.id)
        if (attempted) {
...
exports.getQuizScore = async (req, res) => {
    ...
    try {
        ...
        if (UserInGroup.members.indexOf(req.user.id) === -1)
            return res.status(400).json({ message: 'You are not a member of this group' })
        let attempts = quiz.attempted
        let arr = []
        let found = true
        for (let i = 0; i < attempts.length; i++) {
            // find if req.user.id is in the attempted array
            if (attempts[i].user === req.user.id) {
                let { user: id, noOfAttempts, lastAttempt, score, ...data } = attempts[i]         
                arr.push({
                    id,
                    noOfAttempts,
                    lastAttempt,
        ...
        if (!found) 
            return res.status(400).json({ message: 'User has not attempted this quiz' })
        return res.status(200).json({ arr })
    } catch (err) {
        return res.status(500).json({ message: "Some Internal Error Occured" })
    }
...
exports.deleteQuiz = async (req, res) => {
    ...
    try {
        ...
        if (!quiz)
            return res.status(400).json({ message: 'Quiz does not exist' })
        let Group = await Groups.findById(quiz.group)
        // check if user is admin 
        if (Group.admin !== req.user.id)
            return res.status(400).json({ message: 'You are not an admin of this group' })
        await quiz.remove()
